...
def publish_lap(engine,lap,cumulative,lap_times,cards,season,round_number,track):
 ...
 for pos,car in enumerate(ordered,1):
  ...
  card.update(position=pos,speed=round(3600/max(1,car.last_lap_time),1),gap='LEADER' if pos==1 else f"+{max(0,car.total_time-leader):.3f}s",tyres=car.tyres.compound.value,tyre_wear=round(car.tyres.wear,1),tyre_laps=car.tyres.laps_on,fuel=round(car.fuel_level,1),energy=round(car.energy_battery,1),lap_time=round(car.last_lap_time,3),dnf=car.dnf,progress=getattr(car,'track_progress',0.0),pit_stops=car.pit_stops,health={'engine':round(car.health.engine,1),'gearbox':round(car.health.gearbox,1),'battery':round(car.health.battery,1),'brakes':round(car.health.brakes,1),'suspension':round(car.health.suspension,1),'aero':round(car.health.aero,1),'cooling':round(car.health.cooling,1)})
...
def run_season(number):
 ...
 for round_number,(track,laps) in enumerate(CALENDAR,1):
  ...
  world.update(state='RACE_LIVE',season=name,round=round_number,track=track,lap=0,lap_total=laps,race_lap_seconds=RACE_MINUTES*60/max(1,laps),weather=weather.state_label(),track_temp=round(weather.track_temp,1),air_temp=round(weather.air_temp,1),humidity=round(weather.humidity,1),wind_speed=round(weather.wind_speed,1),track_wetness=round(weather.track_wetness,1),race_control=engine.race_control.flag,cars=cards,message=f'{track} is LIVE')
